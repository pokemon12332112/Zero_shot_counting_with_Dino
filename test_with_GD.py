...
with open(class_dir) as f:
    for line in f:
        ...
        file_contents[img_path] = classes[0]
...
CONFIG_PATH = './groundingdino/config/GroundingDINO_SwinT_OGC.py'#'C:/Users/Admin\Documents/GroundingDINO-main/groundingdino/config/GroundingDINO_SwinT_OGC.py'
CHECK_POINT_PATH = './weights/groundingdino_swint_ogc.pth'#'C:\Users\Admin\Documents\GroundingDINO-main\weights\groundingdino_swint_ogc.pth'
# CONFIG_PATH = args.gd-config
# CHECK_POINT_PATH = args.gd-checkpoint
GD = load_model(CONFIG_PATH, CHECK_POINT_PATH)
def filter_large_boxes(boxes, max_size_ratio=1):
    filtered_boxes = []
...
for im_id in pbar:
    ...
    IMAGE_PATH = im_dir + '/'+ im_id
    #print(IMAGE_PATH)
    TEXT_PROMPT = file_contents[im_id]
    #print(TEXT_PROMPT)
    BOX_TRESHOLD = 0.15
    TEXT_TRESHOLD = 0.25
    ...
    bboxes = box_convert(boxes=boxes, in_fmt="cxcywh", out_fmt="xyxy").numpy()
    bboxes = filter_large_boxes(bboxes)
    # if len(bboxes)>5:
    #     bboxes = bboxes[1:4]
    #rects1 = list()
    rects = list()
